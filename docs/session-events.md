...
`ChatSession.CurrentFormatVersion` is 6 and `SessionEvent.CurrentSchemaVersion` is 3. Unsupported event schemas and old snapshot files are refused rather than guessed or migrated. This Resource Fabric cutover is reset-only: use **Clear Chats/Data** once after upgrading.
