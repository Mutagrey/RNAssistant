...
- `session.commit` applies typed operations such as user/assistant messages, tool calls/results, tool execution boundaries, run state, metadata, context, active references, and artifact revisions. An accepted conversation-response v2 assistant message carries `ResponseProtocolVersion:2` and its exact model-declared `ResponseStatus`; replay never derives either value from `Content`.
