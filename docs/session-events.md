...
- `session.commit` applies typed operations such as user/assistant messages, tool calls/results, tool execution boundaries, run state, metadata, context, active references, and artifact revisions. A current structural response carries `ResponseProtocolVersion:3` and the runtime projection derived from `tool_calls` or a provider-native terminal condition; replay never rescans `Content` wording.
...
- `turn.started` / `turn.ended` delimit one logical user turn. `TurnId` remains stable when a confirmation pause resumes under a new runtime `RunId`. Their data records the run's response protocol version; `turn.ended.Status` is the accepted terminal response status or an explicit runtime-owned failure/cancellation/interruption status.
