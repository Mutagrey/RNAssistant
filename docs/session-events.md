...
Settings → Diagnostics → Trajectory reads the last 500 events from the same stream. Event metadata and state operations are inline; model payloads are fetched lazily by event id and shown as a bounded preview. The bridge never includes API keys or authorization headers.
