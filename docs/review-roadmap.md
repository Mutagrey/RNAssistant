...
3. `web/js/app.js` remains too large and mixes bridge, rendering, settings, model catalog, tools, VBA, context and boot code. This is the next refactor target.
...
- Split `web/js/app.js` into static non-module files loaded in order by `index.html`.
