...
4. A local non-VSTO harness now covers parser, chat storage, fake-adapter pipeline basics, tool catalog composition, context normalization/upsert, prompt trimming/context usage, and a no-network chat completion flow.
...
- Introduce typed bridge request/response DTOs for the largest WebView commands.
