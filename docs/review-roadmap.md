...
- the finite mode/host core stays fixed, while the complete compact exact-id capability catalog scales with the installed catalog and is never silently truncated; prompt-budget validation fails visibly if it cannot fit. Optional exact tool reads are admitted atomically for the next model step only when the complete request fits and the typed event append succeeds, then remain callable for the logical turn without LRU eviction. Confirmation, compaction, and restart replay only the exact accepted turn chain; raw results never restore authority. Skill bodies enter context through the same exact-id reader but retain their separate evidence contract;
