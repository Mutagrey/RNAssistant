...
Для безопасной работы с кодом во всех VBA-capable hosts Agent видит семь общих controller tools: `common.vba_read_module` (без `moduleName` возвращает список компонентов), `common.vba_search_code`, `common.vba_write_module`, `common.vba_apply_patch`, `common.vba_delete_module`, `common.vba_list_backups`, `common.vba_restore_backup`. Host-prefixed `excel|word|powerpoint.vba_*` остаются внутренним COM backend. Старые public ids `vba_list_modules`, `vba_read_lines`, `vba_replace_text`, `vba_create_module` принимаются для совместимости и canonicalize к новому контракту, но Agent их не видит.
