...
Для безопасной работы с кодом во всех VBA-capable hosts Agent видит семь общих controller tools: `common.vba_read_module` (без `moduleName` возвращает список компонентов), `common.vba_search_code`, `common.vba_write_module`, `common.vba_apply_patch`, `common.vba_delete_module`, `common.vba_list_backups`, `common.vba_restore_backup`. Это единственные public VBA ids. Старые `vba_list_modules`, `vba_read_lines`, `vba_replace_text`, `vba_create_module` и host-prefixed варианты удалены. Host-prefixed COM backend не публикуется в tool catalog и вызывается только runtime/typed bridge.
...
`common.vba_read_module` без диапазона читает весь bounded source, а с `startLine`/`lineCount` — точный диапазон до 500 строк. `common.vba_write_module` получает полный source и по умолчанию делает upsert: существующий компонент обновляется с backup, отсутствующий создаётся. Он не переименовывает компонент: новое отсутствующее `moduleName` означает создание отдельного компонента. Переименование нельзя эмулировать через write+delete; для него нужен отдельный атомарный contract с guard/journal сразу для исходного и целевого имени. Если в текущем chat уже был read/search этого модуля, runtime сам сравнивает сохранённый snapshot перед whole-source write: при drift он один раз требует осознанно перечитать/слить изменения либо повторить вызов как намеренную полную перезапись — hash модели не передаётся. Optional `mode=createOnly|updateOnly` сохраняет строгую семантику там, где она действительно нужна. `componentType` используется только при создании. Синтаксически неверное новое имя нормализуется в стабильный ASCII VBA component name длиной до 31 символа с коротким детерминированным hash-suffix; это избегает коллизии с обычным валидным именем, а повтор того же вызова обновляет тот же компонент. Результат всегда возвращает фактическое имя.

Поиск поддерживает literal/regex, timeout и bounded output. `common.vba_apply_patch` изменяет только существующий компонент: отсутствие модуля возвращает structured recovery с `creationTool=common.vba_write_module`; patch не создаёт пустой компонент, потому что anchors и line coordinates требуют исходного source, а тип и полный source нового компонента принадлежат whole-source upsert. `patch` передаётся native JSON array. Model-facing schema использует discriminated `anyOf`: каждая из операций `replace`, `replaceAll`, `replaceFirst`, `insertBefore`, `insertAfter`, `replaceLines`, `regexReplace` содержит только релевантные ей поля. Replacement content задаётся только полем `text`. Literal fragments приводятся к переносам текущего модуля, `replace` требует одно совпадение, insertion anchors должны быть непустыми и уникальными. `insertBefore`/`insertAfter` всегда вставляют блок до/после всей строки с уникальным anchor — даже если anchor совпал только с частью строки — и нормализуют только граничный перенос, не разрезая VBA statement. Insertion не используется вместо replacement; координаты последовательных line operations считаются по результату предыдущей операции, один финальный перенос в replacement text считается terminator. `replaceLines` также корректно обрабатывает модуль с финальным newline и append на `lineCount+1` без фантомной пустой строки. После range-read предпочтителен `replaceLines`, а не большой multi-line literal. Raw control characters запрещены; значения вроде vertical tab формируются выражением `ChrW$(11)` во время выполнения VBA. Mutation сам читает текущее состояние; отдельный read нужен только модели для понимания кода, не runtime для разрешения операции. Перед изменением существующего source или удалением создаётся backup, результат write/patch возвращает фактический read-back hash.
