...
`AgentResponseParser` accepts either a final `message` or one or more `tool_calls` entries with ids unique within that response. `json_schema` derives a strict response contract from the current runnable tools; `json_object` uses the same envelope with local validation. Invalid output gets bounded 1–20 ephemeral format-correction attempts; each starts from clean accepted history and invalid content never enters replay. `OfficeToolExecutor` remains the authority for formal argument schemas, effective pipeline safety, confirmation, and dispatch. `AgentJsonProtocol` serializes each result to `{ok, tool_call_id, name, status, message, data, error}` and emits the selected replay role.
