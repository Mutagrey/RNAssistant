...
- Native tool authority lives in immutable `ToolPolicy`; `LegacyToolDefinitionAdapter` retains existing mutation/confirmation/AgentCanRun restrictions for active legacy consumers. Tool-call status is separate from observed dispatch/effect evidence. Compact evidence survives Activity materialization/replay without duplicating the result payload. [ADR-0003](decisions/ADR-0003-tool-result-three-states.md) and [Phase 4A](stabilization/PHASE_4A_TOOL_RUNTIME.md) define the temporary adapters and separate 4B model-result wire gate. Prompts cannot bypass local authority.
