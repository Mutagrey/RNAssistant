...
- Public VBA editing is a host-neutral `common.vba_*` facade; host-prefixed ids are internal COM backends. Backup, ownership, runtime-bound snapshot, and stale-state protections remain inside VBA execution; see [vba-tool-packages.md](vba-tool-packages.md).
