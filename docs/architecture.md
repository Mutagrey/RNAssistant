...
- Current-turn image, scanned-PDF, and audio attachments use the first compatible model from the saved attachment priority. Routing is request-scoped and does not mutate the chat/global model; ordinary turns return to the selected chat model. There is no endpoint/network failover.
