...
- Model-facing tools are grouped by user intent, not backend COM primitives. Create/update are one upsert only when they share one natural payload and idempotent behavior; operations with materially different payload, partial-update, destructive, or confirmation semantics remain separate instead of using a large union schema.
