...
- Resource model context has one rule: semantic find first, explicit target read second. Every request carries at most a bounded target manifest; local paths, bodies, exact refs and internal ids are omitted. Paste/drop/paperclip still durably link CAS bodies and revisions before model dispatch. Chat/Plan/Agent use `common.resources_find/read`; runtime maps targets to exact canonical refs, applies live hash/collection guards and reconstructs `next` from durable accepted results. Model Tool Results, historical replay, compaction input and media provenance omit URI/revision/cursor state. Text/source chunks are fixed and bounded; media is request-local to the immediate logical step and then released. Live provider calls remain document-identity guarded and serialized with Office mutations.
