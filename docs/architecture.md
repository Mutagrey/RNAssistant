...
- `AgentRouteResolver` хранит только явный runtime mode (обычный Agent или включённый HTML workspace); он не получает и не анализирует текст запроса. `ToolCatalogSlicer`, `PlannerPromptComposer`, `AgentActionValidator`, and `ObservationNormalizer` own metadata slicing, prompt composition, validation, and observations.
...
- Agent mode uses AgentDecision v1. Each turn is exactly one raw JSON object or one native OpenAI `tool_calls[]` response. A tool decision contains 1–8 calls; multi-call batches are restricted to independent read-only tools and execute locally in order. Mutations, local-state changes, confirmation-requiring actions and calls depending on earlier results remain single-call. A run may declare one initial canonical plan; replanning is then disabled and every next turn must select a concrete action. Duplicate successful reads are rejected, and completed parameterless context/summary tools disappear from `AVAILABLE_TOOLS` until a mutation makes the prior observation stale. Fences, surrounding prose, alternate root envelopes and legacy `function_call` are rejected.
...
- There are only two persisted chat modes: `Agent` (default) and `Chat`. In Agent, the model decides from the request whether to answer or select a tool; the runtime does not classify task wording. Chat is a transparent plain completion path with its own `ChatSystemPrompt`; it performs no JSON/thought-envelope repair, while provider reasoning remains separate metadata. Explicit HTML mode and pending agent continuations force Agent.
...
- Deterministic verification uses an exact result/`VerifyJson`/built-in mapping and has a 15-second runtime timeout. A broad workbook/document summary is never synthesized as generic verification; when no exact verifier exists, the successful mutation result stands. A timeout ends the run with a diagnostic instead of starting another COM operation against a potentially blocked Office host.
...
- Normal Agent requests do not capture Office content eagerly. The safe host tool catalog remains available, and the model obtains document-dependent state through an explicit read tool only when needed.
...
- Tool safety belongs to `ToolDefinition` metadata: `MutatesDocument`, `MutatesLocalState`, `AgentCanRun`, `RequiresConfirmation`, risk/capability fields, and verification metadata. `AgentCanRun=false` does not hide a tool; it makes an agent mutation wait for confirmation. Pipeline effective safety recursively includes nested steps and fails closed for missing tools, malformed definitions, duplicate step ids, and cycles. Built-in/controller ids take precedence over custom definitions.
...
- A pending request and the new user message are passed in separate labeled sections without language/length heuristics. The latest visible plan is restored with a no-plan continuation constraint, so it is not duplicated in a new run.
...
- Explicit runtime modes and unfinished visible plans reject premature `final`; ordinary Agent requests rely on the model's `tool` versus terminal decision. Format repair and required-action correction have separate one-shot guards.
...
- A terminal model answer does not mark unfinished plan steps complete. After the initial plan, the response schema excludes another plan for the remainder of the run. A model that ignores this enters bounded format repair; persistent violations stop the current run while preserving the single visible plan and pending task, without adding plan spam to the transcript.
...
- agent runtime guards for strict decision repair, sliced tools, waiting confirmations, max iterations, max tool steps, and fail-closed mutation verification/recovery;
- VBA source is never injected into the prompt automatically: the agent lists project components with the host `vba_list_modules` tool and reads only required modules with `vba_read_module`;
