...
- A v3 chat session JSON remains the canonical transcript and artifact metadata record. Immutable HTML artifact bodies live in session-scoped `html-artifact-bodies` sidecars, are hydrated on load, copied independently on fork, and removed by prune/chat/document/runtime cleanup. Supported v1/v2 inline bodies migrate on the next save. Small fingerprinted `.summary.json` sidecars provide list/lookup metadata, are updated atomically after a session save, and are rebuilt lazily from supported canonical sessions when missing, stale, or invalid.
