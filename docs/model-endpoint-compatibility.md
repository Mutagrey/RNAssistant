...
- SSE streaming for Chat mode when `StreamResponses` is enabled. Text deltas are displayed incrementally in the chat; SSE is detected from the response body even when a compatible endpoint omits or mislabels `Content-Type`.
