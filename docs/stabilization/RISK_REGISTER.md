...
| R23 | P2 | Legacy ToolResult не всегда различает частичный/неизвестный effect; успешный mutating call может быть no-op или иметь слабую domain verification | ToolRuntime / Domains | 3B2 LegacyToolOutcomeAdapter сохраняет partial/missing/uncertain как unknown; counts — top-level вызовы, не document diff; заменить adapter typed evidence Phase 4, domain qualification Phases 6/7 | documented 1C; open |
...
| R27 | P1 | Schema mismatch / UI save могли молча заменить custom prompts или подтвердить старую схему | ModelProtocol / Settings | R29: v4 defaults/schema 13, saved schema 12 сохраняет text/marker при ordinary/failed save; только explicit review/reset подтверждает новые instructions. Shared guard до preparation/confirmation | fixed host-neutral; production controller/WebView/DPAPI qualification pending |
...
| R31 | P2 | Встроенный common.prompt_authoring всё ещё требует unique call id и может вернуть противоречащую v4 инструкцию | Prompts / ModelProtocol | 4B prompt-consumer switch: убрать model-ID guidance, regression на тело skill; runtime IDs/parser R29 не менять | confirmed source review 4A; live skill incident not reproduced, fix deferred to 4B |
