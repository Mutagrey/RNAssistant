...
Phase 2C3B удаляет destructive prompt reset и вводит explicit review.
Единственный active wire/history path всё ещё v2, switch/delete — 2C3C.
...
Подготовительные выделения выполняются только в своей фазе по [§15.2](STABILIZATION_MASTER_PLAN.md#152-рефакторинг-который-облегчает-миграцию). Указанные ниже точки проверены чтением кода; это ориентиры для проверки актуальных consumers перед работой, не разрешение начать Phases 3–7 из Phase 2.
...
| NormalizeAgentPrompts mismatch reset, duplicate Chat/Plan defaulting, UI marker fallback/omitted Plan prompt | Preserve authored text + explicit schema review | Settings / ModelProtocol | SettingsService, typed saveSettings bridge, settings/prompt UI; controller start/confirmation и neutral loop guard | Phase 2C3B: destructive branch/obsolete reset test удалены, callers переключены. Новый prompt marker только с coordinated switch 2C3C; Windows controller/DPAPI qualification остаётся | removed destructive path; permanent review precondition, не adapter |
...
helper имеет ближайший removal gate 2C3C выше. Dual-write не вводится.
