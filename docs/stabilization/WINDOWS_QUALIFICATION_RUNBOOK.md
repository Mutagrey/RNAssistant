...
| WQ-PACK | Phase 8 | revision-pinned resources; stale cursor; bounded large result + exact `ResourceRef`; полный Excel/VBA core в Agent; optional batch даёт одну новую revision без eviction; rejected overflow показывает `TOOL_PACK_STATE` и не меняет прежний pack; pinned execution/callable ToolPack при confirmation/compaction/crash; handler/policy change под тем же ID |
