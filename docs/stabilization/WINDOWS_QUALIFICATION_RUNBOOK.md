...
| WQ-UI | Phase 9 / R28/R32/R46 | restart/replay даёт тот же outcome; pending confirmation; causal navigation request→attempt→call→dispatch→effect; event port сохраняет exact type/correlation/CAS payload, chat-bound hydration отклоняет чужой event; mandatory request/rejected/ToolPack append failure останавливает свой boundary, optional causal/accepted-marker failure outcome не меняет; JSON/diff raw copy; incomplete/error states; stale projection; WebView keyboard/focus/DPI/clipboard; streaming + repair reset |
