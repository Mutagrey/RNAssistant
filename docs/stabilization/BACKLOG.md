...
| 8 | Resource read plane / immutable ToolPack | **8A done host-neutral:** immutable execution snapshot pins descriptor/schema, policy, binding/scope/host and package fingerprint. **8B done host-neutral:** finite Excel/VBA core, atomic full-budget optional admission/new revision and no LRU/partial publication; reconstruction fails closed rather than inferring admission from raw evidence. **8C+:** durable extension event, confirmation/compaction/crash rematerialization and remaining resource handlers without kernel/CAS transport changes. [8A](PHASE_8A_TOOL_PACK_SNAPSHOT.md), [8B](PHASE_8B_CALLABLE_TOOL_PACK.md) |
