...
| 9A / R37 | Accepted call ошибочно сохранялся как result при ненативной instruction role | writer fixed host-neutral по runtime-owned `AcceptedCallOrigin`; 9C UI использует эту typed projection. Read-only adapter ранее затронутых current-v4 streams сохраняется до Windows qualification и explicit retained-data reset/removal, без history rewrite |
...
| 11 | Optional contours | Ранние Artifact Library и 11T typed Office tools contours явно допущены параллельно WQ, без расширения Phase 12. 11A lifecycle/Library, 11B exact Plan, 11C complete HTML и 11D1 bounded text/Markdown viewers done host-neutral. Tool route: real WQ0 → production 5B2 → 7D → parity-first Excel families → independently bound/qualified Word, PowerPoint and Outlook families; каждый switch удаляет свой legacy path, а expansions идут отдельно после evals. Пока Windows gate недоступен, следующий независимый slice — 11D2 image, затем PDF/audio. Далее: coherent Library/Issue Center → read-only selected-endpoint Tool Inspector → Excel Host Fabric → admitted host endpoints → unified UX → custom Tool Library → Skills → Browser/Local Automation. Pipelines remain disabled. [Master](STABILIZATION_MASTER_PLAN.md#phase-11--optional-contours), [Artifacts](../artifact-library.md), [11D1](PHASE_11D1_TEXT_MARKDOWN_VIEWERS.md), [Tools](../tool-library.md), [migration](MIGRATION_MAP.md), [Host Fabric](../host-fabric.md). Runtime authoring and optional VBA adapter removal remain after host pinning/5B2; Phase 6 package execution does not block Phase 12. |
