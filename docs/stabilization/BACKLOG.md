...
| 9A–9C / R32 | Сквозной журнал запуска и общий JSON viewer | **9A/9B1/9B2 read-only inventory done host-neutral через Markdown JSON 9B2B4; следующий R36, затем 9B3.** [Требования](R32_DIAGNOSTICS_JSON_VIEWER.md), [vendor/UI evaluation](R32_VENDOR_UI_EVALUATION.md): correlated query → lossless bounded viewer → раскрываемый журнал. 9B3/9B4/9C отдельны; Windows/WebView gate открыт |
