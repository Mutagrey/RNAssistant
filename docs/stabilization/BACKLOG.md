...
| 9A–9C / R32 | Сквозной журнал запуска и общий JSON viewer | **9A/9B1 и read-only consumers через Artifact JSON 9B2B3 done host-neutral; следующий 9B2B4 Markdown JSON.** [Требования](R32_DIAGNOSTICS_JSON_VIEWER.md), [vendor/UI evaluation](R32_VENDOR_UI_EVALUATION.md): correlated query → lossless bounded viewer → раскрываемый журнал. Markdown, 9B3/9C отдельны; Windows/WebView gate открыт |
