...
| WQ-A | In-app Qualification Center и расширяемые host packs | WQ-A0–A4 завершены host-neutral: A3 добавил единый identity owner/helper и `excel.wq0.identity`; A4 — closed versioned common/provider/storage/UI/Excel/VBA/cross catalog, runner-owned fixture steps и exact fail-closed readiness/coverage gates. Отсутствующий production adapter/environment остаётся N/A. Windows/live evidence не закрыто. [Contract](../qualification.md), [A3](WQ_A3_EXCEL_WQ0.md), [A4](WQ_A4_SUITE_CATALOG.md). Next WQ-A5: immutable build provenance и complete release admission |
