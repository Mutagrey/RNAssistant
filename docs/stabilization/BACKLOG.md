...
| 10 | Финальная структурная сверка и architecture tests | **10A done host-neutral:** six dependency checks + exact inventory. Далее отдельные atomic groups: 10B1 host identity, 10B2 VBA host helper, 10C application façade/resource projection cleanup, 10D final docs/migration check. Optional/5B2/7D consumers не трогать. [Evidence](PHASE_10A_BOUNDARY_AUDIT.md) |
