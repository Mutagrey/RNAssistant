...
| 9A–9C / R32 | Сквозной журнал запуска и общий JSON viewer | **9A + 9B1 done host-neutral; следующий отдельный 9B2 diagnostics switch.** [Требования](R32_DIAGNOSTICS_JSON_VIEWER.md), [vendor/UI evaluation](R32_VENDOR_UI_EVALUATION.md): correlated query → lossless bounded viewer → раскрываемый журнал. 9B2/9B3/9C отдельны; Windows/WebView gate открыт |
