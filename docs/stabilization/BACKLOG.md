...
| 6 follow-up / R33 | Перекрывающиеся exact-match вхождения | source review: `aaaa` / find=`aaa` имеет два стартовых смещения, текущий non-overlapping counter считает одно. Отдельный semantic fix + targeted text/integration coverage до полного VBA gate; не исправлялся в сохраняющем поведение 6A |
| 6 remainder | VBA vertical slice; отдельно оценить необходимость пользовательского package lifecycle для stable | Raw CAS bytes отдельно от comparable normalization; exact patch, journal/read-back/fault matrix, no fabricated terminal при persistence failure; общий package journal нужен rename, scope пока не сокращён |
