...
| 8 | Resource read plane / immutable ToolPack | **8A done host-neutral:** immutable execution snapshot pins descriptor/schema, policy, binding/scope/host and package fingerprint. **8B done host-neutral:** finite Excel/VBA core, atomic full-budget optional admission/new revision and no LRU/partial publication. **8C done host-neutral:** accepted/rejected typed events form a durable-before-publication barrier; the exact accepted turn chain rematerializes across confirmation/compaction/restart, while raw/rejected evidence grants no authority. **8D+:** remaining resource handlers/R30 without kernel/CAS transport changes. [8A](PHASE_8A_TOOL_PACK_SNAPSHOT.md), [8B](PHASE_8B_CALLABLE_TOOL_PACK.md), [8C](PHASE_8C_TOOL_PACK_EVENTS.md) |
