...
| 6 remainder | Package/rename scope and ownership | Next separate 6H starts with consumer/stable-core scope decision required by the master plan; then only admitted package/rename slices. Raw CAS stays separate from comparable text; package lifecycle is not removed or migrated without the explicit decision |
