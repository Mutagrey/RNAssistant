...
| 6 remainder | Typed VBA outcome/fault boundary, remaining mutations and package scope | Next separately admitted 6D: typed request/outcome, mapping only in executor, remove string rollback classification, terminal-persistence/fault matrix with no fabricated terminal/retry. Then whole-module write/delete/restore and package/rename ownership in ordered slices; raw CAS stays separate from comparable text, package scope not reduced without consumer decision |
