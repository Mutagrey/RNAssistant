...
| WQ-A | In-app Qualification Center и расширяемые host packs | WQ-A0–A3 завершены host-neutral: A3 добавил один `OfficeHosts.Qualification` identity owner, narrow same-build x64 helper, typed host port и embedded release pack `excel.wq0.identity`; duplicate diagnostic sources удалены. Windows WQ0 evidence открыто, поэтому 5B2/7D/R04 не закрыты. [Contract](../qualification.md), [A1](WQ_A1_QUALIFICATION_CORE.md), [A2](WQ_A2_QUALIFICATION_CENTER.md), [A3](WQ_A3_EXCEL_WQ0.md). Next WQ-A4: versioned suites без script execution, second store/executor или model/UI-owned pass |
