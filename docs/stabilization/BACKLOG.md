...
| 9 | Persistence / UI projection | 9D1 audit done: один stream/CAS и `IRunStore` сохраняются; existing replay/barrier/fault coverage mapped. Next 9D2 closes R45 same-process fail-stop reload/reconciliation; затем 9D3 typed event classification/`IEventStore`, 9D4 `IConversationStore`, 9D5 immutable `RunViewState` и stale/multi-window coverage. [Evidence](PHASE_9D1_PERSISTENCE_AUDIT.md) |
