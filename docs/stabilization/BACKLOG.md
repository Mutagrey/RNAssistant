...
| WQ-A | In-app Qualification Center и расширяемые host packs | WQ-A0–A2 завершены host-neutral: strict core подключён через application/controller/bridge к empty-chat/Diagnostics UI, dedicated event-backed chat и exact read-only `common.ui-shell`; restart/resume, journal/JSON navigation и bounded report проверены. [Contract](../qualification.md), [WQ-A1](WQ_A1_QUALIFICATION_CORE.md), [WQ-A2](WQ_A2_QUALIFICATION_CENTER.md). Next WQ-A3: Excel WQ0 owner/helper/Windows qualification. Никаких script execution, second store/test executor или model/UI-owned pass |
