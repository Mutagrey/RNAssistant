...
Latest slice (2026-09-06): all existing image/PDF/thumbnail/raw binary consumers use 256 KiB sequential chunks through the same resource leases and shared 50 MiB transfer budget. Whole-payload limits remain separate. CAS is verified once per lease, buffers release on completion and busy close retains capacity until exit. Artifact viewers accumulate/hash-check chunks into revocable blob URLs; RN.resources streams byte offsets; export manifest v2 stores chunk parts without v1 fallback. Focused host-neutral checks cover chunk order/bounds, shared quota, close during capture, exact raw/image/PDF behavior and browser lifecycle/offline export. Next: remaining search/collection consumers and other-provider raw view gaps, then final cleanup. Windows/WebView2 qualification remains open; Phase 12 is not started.
