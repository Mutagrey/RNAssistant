...
Current task: WQ-A2 done host-neutral — application/controller/bridge composition, dedicated event-backed qualification chat, empty-chat/Diagnostics UI, durable resume, shared journal/JSON navigation и bounded report подключены к exact read-only `common.ui-shell`. Этот pack не квалифицирует Office/model/document tools.
...
| WQ-A | A0–A2 done host-neutral; A3–A5 pending | [contract](../qualification.md), [WQ-A1](WQ_A1_QUALIFICATION_CORE.md), [WQ-A2](WQ_A2_QUALIFICATION_CENTER.md), [ADR-0010](../decisions/ADR-0010-qualification-evidence-authority.md) | A2: qualification 10/10; web 5/5; Harness/MockDemo compile; restart/bridge/browser smoke pass | not performed | UI shell exposes only read-only `common.ui-shell`; production Agent/host packs, WQ0 and immutable build evidence remain open |
