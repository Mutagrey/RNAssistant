...
Current task: 11T9C2 is done host-neutral: all four `common.plan_doc_*` mutations use exact native Plan-only handlers and no Plan Document controller executor. 11T9C3 Task List family is next. Real Windows/live-provider gates remain open.
...
| 11 | in progress; 11T0–11T9B + 11T9C1–11T9C2 done host-neutral, remaining native controller/authoring migration and final legacy cleanup mandatory | 11A–11D1 evidence above; [11T9C2](PHASE_11T9C2_PLAN_DOCUMENT_NATIVE_RUNTIME.md) | 11T9C2: Plan mode 3/3; Plan Document 3/3; ToolPack 6/6; architecture 4/4; source inclusion 1/1; MockDemo build | not performed | 11T9C3 Task List next, then HTML/capabilities/prompts, typed authoring and final generic-dispatch/adapter removal before Phase 12 |
