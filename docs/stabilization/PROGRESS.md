...
Latest slice (2026-09-06): excel.find_cells now captures complete bounded workbook/sheet/range/selection cell snapshots through the existing Excel provider/Gateway/CAS, then applies pure domain matching. Direct adapter search and public hash/full-cell output are removed; pattern/formula/coordinate semantics remain. Exact pages, zero-match drift, replacement invalidation, pre-materialization cell-count bounds, invalid capture refusal and bound-STA/closed-workbook checks pass host-neutral. Generic resources_find and all four host search consumers are now switched host-neutral. Next: remaining provider raw-view gaps and final consumer/retention cleanup in MASTER order. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
