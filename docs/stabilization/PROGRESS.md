...
Latest slice (2026-09-06): word.find_text preserves literal/regex/main/selection/all semantics but captures exact story-scope resources through Gateway/CAS and then runs pure Word matching. The direct adapter read/search path is removed; search results carry complete evidence rather than public scope/content hashes. Positive and zero-match search, historical pages after drift, mutation invalidation, bounds before text capture and bound-STA/closed-document paths pass host-neutral. Next: generic resources_find scan publication (including no matches; recorded in RISK_REGISTER), then remaining Excel/PowerPoint/Outlook search consumers and other-provider raw gaps before final cleanup. Windows Word/COM/WebView2 qualification remains open; Phase 12 is not started.
