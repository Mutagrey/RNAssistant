...
Current task: user-requested R61/11O all-tool correction has started. 11O0 source-built-in property inventory is complete host-neutral: 73 unique built-in ids / 76 effective host variants are pinned by exact descriptor revision, mode, binding and recursive properties. Runtime schemas, skill bodies and Library UI are unchanged; dynamic custom-package review and every family cutover remain open. The exact Windows rebuild/error-path retest and remaining live-provider gates are also open.
...
Next step for tools: 11O1 characterizes and then atomically switches the Resources + Capabilities family to minimal semantic intent over the existing ToolRuntime. It must remove model-owned provider/kind/URI/revision/cursor/offset/page-size choreography, switch Chat/Plan/Agent callable profiles and every affected prompt/skill consumer together, and preserve internal fail-closed revision/continuation evidence without aliases. Exact ids and continuation UX are accepted only by the targeted comparative scenarios in the audit. The accumulated Windows rebuild/R62 retest remains mandatory before final WQ evidence.
Required context: [R61 Tool Library contract](../tool-library.md#mandatory-all-tool-contract-audit-r61), [failure/ownership audit](R61_TOOL_CONTRACT_AUDIT.md), [exact built-in property inventory](R61_TOOL_PROPERTY_INVENTORY.tsv), [conversation protocol](../conversation-protocol.md), [Resource Fabric](../resource-fabric.md), [11T10 evidence](PHASE_11T10_ACTIVE_LEGACY_REMOVAL.md), [qualification contract](../qualification.md) and exact Resources/Capabilities catalog, handler and prompt/skill consumers only.
Open gates / remaining legacy: active tool execution/catalog/result legacy is empty. All current Excel, Word, PowerPoint, Outlook, public VBA/macro, custom VBA package, controller-owned tools and Tool/Skill model/UI authoring use direct typed owners. `ThisAddIn` active-window/document lookup remains only VSTO pane lifecycle discovery, never execution target fallback, and is part of Windows UI/session qualification. Permanent narrow journal ports and current model-compatibility diagnostics share the canonical authority and are not legacy. R61 11O0 source-built-in inventory is complete; dynamic custom-package property review, all schema/skill family switches, UI-only system documentation and typed Library Test/Implementation layout remain open. R62 is fixed host-neutral but requires the exact Windows WebView model/tool-error retest. R51 remains open for image/PDF/audio viewers, other committed-resource removal and Windows WebView lifecycle. Production OfficeHosts/VSTO build, actual COM marshal/cleanup, real DocumentSession lifetime, WQ0, WQ-SESSION, WQ-EXCEL, WQ-WORD, WQ-POWERPOINT and WQ-OUTLOOK are open evidence. Full Phase 6 Windows/VBE, Phase 8 WQ-PACK, Phase 9/R45–R48 WebView/restart/multi-window and R28/R29/R32 live-provider/UI gates remain open. R52 Host Fabric, R53 Local Automation, R54 Skill Library, R56 Tool Library, R57 Issue Center and R58 typed-facade risk remain open. Product 16.1.0-dev, no release/tag.
...
| 11 | existing-tool migration route done host-neutral through 11T10; R61 semantic contract correction pending | 11A–11D1 evidence above; [11J1](PHASE_11J1_TOOL_AUTHORING_NATIVE_RUNTIME.md), [11J2](PHASE_11J2_VBA_PACKAGE_NATIVE_RUNTIME.md), [11K1](PHASE_11K1_SKILL_AUTHORING_NATIVE_RUNTIME.md), [11K2](PHASE_11K2_SKILLS_UI_TYPED_BOUNDARY.md), [11T10](PHASE_11T10_ACTIVE_LEGACY_REMOVAL.md) | 11T10: full harness 589/589; Tool Library web 6/6; architecture/source inclusion and MockDemo pass | not performed | Active legacy removed; separate mandatory R61 all-tool schemas/docs/test UX remains before final Milestone WQ |
| 11O / R61 | pending; docs-only target accepted | [Tool Library contract](../tool-library.md#mandatory-all-tool-contract-audit-r61) | source audit only; runtime tests not run | not performed | Per-tool intent/runtime ownership, UI-only built-in docs, typed Test form and Implementation/Test layout |
