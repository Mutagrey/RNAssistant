...
Latest slice (2026-09-06): raw now admits committed CAS originals for file/Markdown/plan/task-list/chart artifacts through the existing binary owner, Gateway, authority retention and 256 KiB data-plane chunks. No attachment reader is required for these stored originals; the 20 MiB bound and inert MIME remain. Missing CAS, malformed metadata and broken attachment provenance fail closed, without InlineText hydration or serialization fallback. Exact byte preservation, historical reads, empty sources and admission guards pass host-neutral. Generic resources_find and all four host search consumers remain switched host-neutral. Next: remaining workspace-member/provider raw views and final consumer/retention cleanup in MASTER order. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
