...
Latest slice (2026-09-07): Defined Names now use the existing Excel provider/Gateway/CAS via `Excel name: <exact name>`, preserving global/sheet-qualified identity. Only typed proof of a single range in the bound workbook permits cells; unresolved/foreign/multi-area names are metadata-only. Exact sources include definition and extent, so either kind of drift supersedes prior evidence; retained text/records survive live name removal without Office I/O. Fifteen focused checks pass (names, existing reads/tables/search, Gateway, architecture and project inclusion), including bound-STA/closed-session routing and fail-closed admission. Real Windows COM name classification and dynamic/external/multi-area behavior are unqualified. Next: finer Excel impact/coverage and remaining consumer audit, then cold-replay/retention and final documentation cleanup in MASTER order. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
