...
Latest slice (2026-09-06): built-in human documentation now belongs to the existing catalog publication and CAS. The Library reads its exact revision-pinned resource through shared bounded download with an explicit chat/Library-revision guard; inline Markdown and cumulative documentation caches are removed. The selected cache retains the exact resource, cancellation closes late leases, and unavailable/corrupt snapshots never fall back to generation. Next: remaining definition/domain reads; the observed canonical/raw VBA snapshot collision remains recorded for a focused authority correction. Windows gates stay open.
