...
Latest slice (2026-09-06): generic resources_find live document/VBA scans now publish captured source views independently of matches. Zero-match scans publish drift; matching snippets bind to those same logical revisions. Complete captures support historical CAS reads without Office I/O; truncated VBA prefixes keep partial coverage, and backup metadata never invents source evidence from a hash. The snippet-only publication path is removed. Focused search/Gateway regressions pass host-neutral. Next: remaining Excel/PowerPoint/Outlook search consumers and other-provider raw gaps before final cleanup. Windows/COM/WebView2 qualification remains open; Phase 12 is not started.
