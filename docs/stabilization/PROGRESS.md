...
Next step for tools: establish integrated two-chat mutation/no-op/unknown/guard acceptance using the existing native route and compiler, per the [MASTER acceptance reconciliation](../resource-fabric.md#master-acceptance-reconciliation--2026-09-07). Then resolve the named source-allocation qualification limits and finalize removal records. The catalog publication/final-freeze race is fixed host-neutral. No blanket provider expansion, finer Excel coverage or cold-replay/checkpoint optimization is scheduled. Final Windows rebuild/live-provider/WebView2 qualification, including accumulated R61–R77 retests, remains required; Phase 12 is blocked.
Latest slice (2026-09-07): active catalog generation is required for authority-backed model-session creation and supplied on rebind. Final compilation checks that generation against its same frozen authority tuple; intervening publication fails with RESOURCE_CATALOG_CHANGED before request dispatch or receipt publication. Fresh binding permits progress; existing request/repair keeps original catalog/settings/budget. No second builder, fresh-head substitution or automatic replay. Eight focused checks pass: extended frozen-prompt race regression, prompt/catalog composition, normal write/result run, compaction rebuild, durable admission, compiler correctness order, immutable skill publication and architecture. The finite acceptance list from ab01107 remains in force; real Windows qualification is open.
