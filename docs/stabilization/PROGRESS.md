...
Latest slice (2026-09-06): HTML workspace file/data Save/create now use one shared upload writer. Inline source arguments are removed from all four UI callers and strict typed bridge payloads. Complete UTF-8 source enters the existing CAS-backed intent and HTML mutation/commit owner; exact displayed-workspace and Known logical-head guards run before dispatch. Invalid uploads and stale drafts cannot write, and Save acknowledgement preserves late edits without silent rebase/replay. Next: outgoing workspace source/preview hydration, remaining definition/domain reads and bulk upload/export. Windows gates and separate R61/VBA/storage gaps stay open; an unrelated stale VBA cache assertion in the Plan Web fixture is recorded in BACKLOG.
...
Next host-neutral contour: outgoing workspace source/preview hydration, remaining definition/domain reads
