...
Latest slice (2026-09-06): model tool-source inspection now uses common.resources_find/read and the same published catalog source children as the Library. Metadata-only host-visible discovery, bounded exact source and root-dependent evidence replace common.tools_definition_read and its direct file/native path. Reading cannot admit a callable tool; later publications supersede prior evidence while historical snapshots stay readable. Next: prompt inspection and remaining domain reads. The broader R61 inventory baseline gap and canonical/raw VBA collision are recorded separately; Windows gates stay open.
...
Next host-neutral contour: prompt inspection and remaining definition/domain reads
