...
Latest slice (2026-09-06): Outlook mail reader consumer switch is complete host-neutral. Header-only bound mail discovery, document/selection/mail-child views and HTML now use the existing Gateway/document provider, authority/CAS and evidence. Removed `outlook.read_mail`, its native binding and output wrapper. Inspector/folder membership is checked before body capture; retained pages never recapture mail. OOM pre-materialization and real Windows/COM/catalog gates remain open. Next: remaining bulk upload/export, search/collection consumers and final cleanup. Prior WebView2 response-threading correction remains committed with its real Windows retest open; earlier PowerPoint/Word/Excel/HTML slices are unchanged.
