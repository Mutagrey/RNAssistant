...
Latest slice (2026-09-06): powerpoint.search_text now captures exact deck/slide shape-and-notes snapshots through Gateway/CAS, then applies pure domain matching. Direct adapter search and public scope/content hashes are removed; literal/regex/notes/coordinates and output limits are preserved. Positive/zero-match/empty scopes retain complete evidence; drift/mutation invalidation, historical no-I/O reads, missing-CAS refusal, malformed/bounded capture and bound-STA/closed-presentation search pass host-neutral. Generic resources_find scan publication remains complete host-neutral. Next: remaining Excel/Outlook search consumers and other-provider raw gaps before final cleanup. Windows/COM/WebView2 qualification remains open; Phase 12 is not started.
