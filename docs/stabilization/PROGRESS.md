...
Current task: 11J1 is done host-neutral: all four `common.tools_*` authoring operations use exact native Agent handlers with guarded verified mutations, and `ToolAuthoringExecutor` is gone. Mandatory 11J2 custom package execution/UI cutover is next. Real Windows/live-provider gates remain open.
...
Next step for tools: mandatory 11J2 переводит existing custom package execution и Tools UI install/remove/status на versioned typed package source/result contracts и удаляет `VbaPackageToolAdapter`/`VbaLegacyResultProjection` из этого контура; затем 11K переводит Skill authoring, а 11T10 удаляет final generic catalog/dispatch and legacy definition/result/UI adapters.
Required context: [master Phase 11J](STABILIZATION_MASTER_PLAN.md), [11J1 evidence](PHASE_11J1_TOOL_AUTHORING_NATIVE_RUNTIME.md), [Tool Library](../tool-library.md), [VBA packages](../vba-tool-packages.md), [migration map](MIGRATION_MAP.md), current `ToolStore`/package execution/UI consumers and focused tool-store/VBA package/UI tests.
Open gates / remaining legacy: 11A1–11A2, 11B1–11B3, 11C1–11C3, 11D1, 11T0–11T9B, 11T9C1–11T9C6 and 11J1 are done host-neutral, but R51 remains open for image/PDF/audio viewers, other committed-resource removal and Windows WebView lifecycle. All current Excel, Word, PowerPoint, Outlook, public VBA/macro, Plan question, Plan Document, Task List, HTML workspace, capability discovery/read, prompt read/save and custom Tool authoring execution uses direct typed native handlers; their replaced compatibility commands/controller executors and execution-time active-document/source fallback are gone. Existing custom package execution/UI and Skill authoring plus final cleanup still must remove the remaining controller enum, `IOfficeApplicationAdapter.GetBuiltInTools/ExecuteTool`, `LegacyToolDefinitionAdapter`, `LegacyToolResultAdapter`, `ToolResultUiProjection` and package/UI projections. `VbaPackageToolAdapter` and `VbaLegacyResultProjection` remain only for custom package/manual/reconciliation UI until 11J2. Permanent narrow journal ports and current model-compatibility diagnostics are not legacy because they do not create a second authority. Mandatory host-neutral route 0–10 and WQ-A1–A5 implementation are complete, while all real Windows/live-provider gates remain open; catalog/evidence admission tests are not scenario evidence. Production OfficeHosts/VSTO build, actual COM marshal/cleanup, real DocumentSession lifetime, WQ0, WQ-SESSION, WQ-EXCEL, WQ-WORD, WQ-POWERPOINT and WQ-OUTLOOK are open evidence, not implementation blockers. Full Phase 6 Windows/VBE, Phase 8 WQ-PACK, Phase 9/R45–R48 WebView/restart/multi-window and R28/R29/R32 live-provider/UI gates remain open. R52 Host Fabric, R53 Local Automation, R54 Skill Library, R56 Tool Library, R57 Issue Center and R58 typed-facade risk remain open. Product 16.1.0-dev, no release/tag.
...
| 11 | in progress; 11T0–11T9B + 11T9C1–11T9C6 + 11J1 done host-neutral, remaining package/UI and Skill migration plus final legacy cleanup mandatory | 11A–11D1 evidence above; [11J1](PHASE_11J1_TOOL_AUTHORING_NATIVE_RUNTIME.md) | 11J1: Tool CRUD 1/1; Tools 36/36; pipelines 3/3; architecture/source inclusion/MockDemo/version evidence in report | not performed | 11J2 custom package execution/UI next, then 11K Skills and final generic-dispatch/adapter removal before Phase 12 |
...
|---|---|---|---|
| Legacy ToolResult → LegacyToolOutcomeAdapter | ToolRuntime | Custom package execution/UI and Skill authoring controller handler → kernel records | Tool authoring bypasses it after 11J1; 11J2/11K remove the remaining consumers; R23 remains |
| LegacyToolDefinitionAdapter | ToolRuntime | Current package/Skill/generic catalog and execution consumers listed in `MIGRATION_MAP.md`; resource and Tool-authoring execution paths removed | 11J2 package, 11K Skill and 11T10 final generic removal; `ProjectRead` removed in 10C2, no resource dependency |
| LegacyToolResultAdapter | ToolRuntime | Custom package execution and Skill authoring controller executor → typed result materialization | Office/VBA/Plan/HTML/capability/prompt/Tool-authoring families bypass it after 11J1; 11J2/11K remove remaining consumers; no old-history reader |
...
| `VbaPackageToolAdapter` | VBA / Tools | Remaining `ToolDefinition` → typed package source mapping for custom package execution/UI | 11J2 typed versioned package source switch; host command/result backend was deleted in 11T9A |
| `VbaLegacyResultProjection` | VBA / Tools | Custom package/manual/reconciliation UI boundaries → legacy `ToolResult` | Public Agent use removed in 11T9B; 11J2 removes remaining UI/package use; no model-history reader |
