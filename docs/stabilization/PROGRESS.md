...
Latest slice (2026-09-06): existing image/PDF binary views now have metadata-only descriptor capabilities and per-view bounds from the media owner. Gateway refuses unsupported views/selectors before capture and enforces the same MIME/size contract on retained views. Six focused host-neutral cases pass, including existing image/PDF and generic Gateway regressions. Next: original-byte raw resource views and bounded transport, then remaining search/collection consumers. Native renderer/WebView2, cleanup and Windows qualification gates stay open.
