...
Latest slice (2026-09-06): Prompts UI source/read-save transport is switched. Init/settings carry body-free typed controls and exact prompt metadata; selected source uses shared Gateway/CAS downloads and changed fields use single-use uploads before the existing settings writer/commit barrier. Unloaded fields are preserved, stale/live-drift writes fail before dispatch, and dirty/late edits survive failures. Inline prompt bodies and whole-form prompt serialization are removed. Next: remaining definition/domain reads and bulk upload/export. R61 inventory baseline drift and canonical/raw VBA collision remain separate open gaps; Windows gates stay open.
