...
Latest slice (2026-09-07): UI delivery and stream cache coherence are fixed host-neutral. The streaming transcript reconciliation fix now ships under a new `app-messages.js` cache key, and the web delivery tests/manifest match the current local-resource CSP and snapshot contracts. Resource authority, bridge contracts, Office binding and Windows/COM/WebView2 gates are unchanged and remain open.
