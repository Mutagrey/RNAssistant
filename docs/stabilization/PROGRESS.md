...
Latest slice (2026-09-06): retained binary metadata now uses the shared exact snapshot reader and bounded typed JSON parsing. Missing/null/corrupt metadata, invalid payloads or mismatched retention parts return RESOURCE_SNAPSHOT_UNAVAILABLE without recapture, rendering or authority publication. Reproduced the former null-metadata exception and removed the null-record recapture branch; 12 focused checks pass, including 22 raw/thumbnail corruption variants. Original-byte view negotiation is covered for implemented attachment/file/JSON/workspace sources; raw is not required for every Office/state/catalog projection. Next: remaining consumer and Excel coverage/named-resource audit, then cold-replay/retention and final documentation cleanup in MASTER order. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
