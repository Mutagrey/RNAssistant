...
Latest slice (2026-09-07): performance hardening step 1 begins with WebView CSS hot-path cleanup. The global `.shell *` scrollbar/state selectors are replaced by explicit scroll-surface selectors for transcript, chat/resource lists, viewers, diagnostics, Library/VBA panels and CodeMirror scrollbars. This is host-neutral UI CSS only; it does not change Resource authority, EventStore, bridge contracts, Office binding or Windows/COM/WebView2 gates. Version-format gate passes.
