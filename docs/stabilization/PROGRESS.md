...
Latest slice (2026-09-07): global catalog projection no longer discovers document VBA during model input/publication refresh. Bound host/document registrations retain their owning input path; explicit document/Library discovery remains guarded and unchanged. The closed-document agent keeps local HTML bindings, performs no Office I/O and still refuses a live Excel read. Focused catalog, frozen-request and two-chat checks pass host-neutral. Prior raw-byte revision fix: f78809f. Final fixture/removal reconciliation remains; Outlook/Inspector allocation is user-deferred and Windows gates stay open.
