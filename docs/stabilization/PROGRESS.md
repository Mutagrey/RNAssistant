...
Latest slice (2026-09-06): outlook.search_mail now captures exact bounded folder field projections through Gateway/CAS, then applies pure domain matching. Direct adapter search, EntryID/repeated-body output and maxBodyChars are removed; semantic mail targets, regex/field coordinates and explicit body/folder truncation replace them. Header-only search does not read bodies. Positive/zero-match/empty scopes retain complete projection evidence; drift, historical no-I/O, missing-CAS refusal, invalid captures and bound-STA/Inspector/closed-window checks pass host-neutral. Generic resources_find and Word/PowerPoint search cutovers remain complete host-neutral. Next: Excel search consumer and other-provider raw gaps before final cleanup. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
