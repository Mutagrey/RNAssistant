...
Latest slice (2026-09-06): PowerPoint document/slide source read cutover is complete host-neutral. The existing Gateway/document provider now uses typed bounded captures, exact CAS views and evidence for text and speaker notes. Removed `powerpoint.read_slides` and its direct output path; oversized decks/slides fail without clipping. Search/object inspection/mutations remain specialized; the existing selection observation is unchanged. Prior Word/Excel/HTML slices remain complete host-neutral. Next: Outlook domain reads, remaining bulk upload/export and final consumer cleanup. All Windows and recorded fixture gates remain open.
