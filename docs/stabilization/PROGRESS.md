...
Latest slice (2026-09-06): WebView2 resource response adapter now marshals response creation/assignment and deferral completion back to the control thread after background resource reads, preventing prompt-reset/local-resource failures from touching CoreWebView2 off-STA. This is host-neutral source correction only; real Windows WebView2 reset/resource retest remains open. Outlook exact-capture prerequisite is complete host-neutral, not the Outlook resource cutover. `CaptureMail` is shared by the existing reader and reply/update preparation: body reads are complete-or-error, attachment-only reads do not touch body, missing/mismatched snapshots and cancellation fail before dispatch. The public reader remains pending replacement. Next: bound Outlook mail identity/discovery and Gateway/CAS consumer switch; remove `outlook.read_mail`/its output wrapper, then remaining bulk transport and cleanup. OOM body materialization and all Windows gates remain open; prior PowerPoint/Word/Excel/HTML slices are unchanged.
