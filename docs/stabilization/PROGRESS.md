...
Latest slice (2026-09-06): Excel table names now have stable resource identities through the existing Excel provider, typed metadata/range backend and Gateway/CAS. Generic discovery/read accepts Excel table: Sales; source snapshots include physical extent so relocation with equal cells advances revision. Table/records at $.values share retained history and HTML delivery; no lookup of the former/current address on historical reads. Complete bounded catalog, unique name, local rectangle and pre-capture cell limit are enforced. Defined Names (external/dynamic/non-range references) and finer Excel coverage remain open; no generic raw expansion is pending. Next: those remaining Excel requirements/consumer audit, then cold-replay/retention and final documentation cleanup in MASTER order. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
