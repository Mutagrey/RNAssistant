...
Latest slice (2026-09-06): HTML workspace source read cutover is complete host-neutral. All init/chat/mutation/export projections contain file metadata and exact member refs, not source text. Selected editor source and complete preview/export source sets use the same Gateway/member provider/CAS/shared download with one bounded producer. Missing or corrupt source cannot become an empty editable file; late reads are closed and dirty drafts retain their original guard across newer metadata. Explicit source reload/retry preserves concurrent edits. Next: remaining definition/domain reads and bulk upload/export. Windows gates and separate R61/VBA/storage/fixture gaps remain open in BACKLOG.
...
Next host-neutral contour: remaining definition/domain reads
