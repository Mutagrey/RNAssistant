...
Latest slice (2026-09-06): model prompt inspection now uses common.resources_find/read for individual current/default fields in the existing catalog. Startup publishes builtin defaults through the same authority/CAS owner; exact bounded reads create root-dependent evidence without loading mutable settings or activating defaults. common.prompts_read and its direct-settings/native branch are removed; save guards and frozen compiler activation are unchanged. Next: Prompts UI transport and remaining domain reads. The broader R61 inventory baseline gap and canonical/raw VBA collision remain recorded separately; Windows gates stay open.
...
Next host-neutral contour: Prompts UI transport and remaining definition/domain reads
