...
Latest slice (2026-09-06): Outlook folder collection uses the existing document provider/Gateway, shared authority and CAS. Generic text plus bounded records/table at $.messages replace outlook.collect_mail, its request/handler/monthly wrapper and replay acceptance. The captured projection covers at most 500 newest folder items with 1000-character body previews, explicit folder/body truncation, one preview capture and aggregate bounds; month remains a semantic field for consumer grouping. Historical records remain pinned after membership drift and HTML binds the same source. Focused Outlook resource/capture/tool checks pass host-neutral. Next: remaining search consumers and other-provider raw view gaps, then final cleanup. Windows/OOM/WebView2 qualification remains open; Phase 12 is not started.
