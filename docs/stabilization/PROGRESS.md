...
Latest slice (2026-09-06): Excel range read cutover is complete host-neutral. Values/formulas/profile now use `common.resources_read` and the existing Gateway/provider/domain capture, exact CAS views and evidence. The old `excel.read_range` catalog/core-pack/native binding and direct output path are removed; implicit active-sheet/selection defaults are not translated. HTML source downloads remain complete from the preceding slice. Next: remaining Word/PowerPoint/Outlook domain reads and bulk upload/export. Windows gates and separate R61/VBA/storage/fixture gaps remain open in BACKLOG.
...
Next host-neutral contour: remaining Word/PowerPoint/Outlook domain reads
