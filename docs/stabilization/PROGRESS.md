...
Latest slice (2026-09-06): Word source read cutover is complete host-neutral. Document/selection and explicit character ranges now use the existing Gateway/document provider, typed Word capture, exact CAS views and evidence. Removed `word.read_text` and its direct output/fallback path; source limits precede COM text materialization, without clipping or implicit range ends. Prior Excel and HTML slices remain complete host-neutral. Next: PowerPoint/Outlook domain reads and remaining bulk upload/export. All Windows and recorded fixture gates remain open.
