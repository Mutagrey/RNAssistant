...
Latest slice (2026-09-06): workspace file members now offer raw UTF-8 source through the existing exact member identity, CAS retention and 256 KiB binary route. All raw capture is owned by ChatArtifactResourceProvider/IResourceRawSource; the viewer raw-reader is removed. Gateway validates source length/hash before publication. Data-binding members do not advertise raw because binding metadata is not bound data. BOM/CRLF/Unicode, source/raw CAS identity, edits with historical reads, empty files, missing-CAS refusal and selector/scope guards pass host-neutral; 14 focused checks pass including image/PDF, editor, HTML binding and architecture. Next: remaining provider view negotiation and final consumer/retention cleanup in MASTER order. Windows/COM/OOM/WebView2 qualification remains open; Phase 12 is not started.
