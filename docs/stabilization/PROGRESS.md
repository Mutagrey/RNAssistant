...
Current task: WQ-A3 done host-neutral — один identity decoder/lease owner перенесён в `OfficeHosts.Qualification`; application wrappers подключают exact typed host port, release selector показывает embedded `excel.wq0.identity`, а same-build x64 helper имеет bounded nonce pipe contract без shell/network. Duplicate test-project decoder удалён. Реальный Windows WQ0 не выполнялся.
...
| WQ-A | A0–A3 done host-neutral; A4–A5 pending | [contract](../qualification.md), [A1](WQ_A1_QUALIFICATION_CORE.md), [A2](WQ_A2_QUALIFICATION_CENTER.md), [A3](WQ_A3_EXCEL_WQ0.md), [ADR-0010](../decisions/ADR-0010-qualification-evidence-authority.md) | A3: qualification 11/11; identity 5/5; web 5/5 | not performed | Exact Excel WQ0 pack/owner/helper wired, but real Windows evidence and production 5B2 remain open; Agent suites/build evidence pending |
