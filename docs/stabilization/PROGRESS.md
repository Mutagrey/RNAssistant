...
Latest slice (2026-09-06): exact attachment-backed raw originals use the existing Gateway/CAS/binary lease and HTML binding/read/export path, without decoding or rendering. Source hash/length, empty files, CAS reuse/missing payload, scope/size/selector refusal and retained-source identity are checked; raw HTTP MIME is inert octet-stream. Eight focused C# and 13 browser cases pass host-neutral. This is a 20 MiB bounded whole response (HTML export keeps its 8 MiB part bound), not chunked binary streaming. Next: general chunked binary transport and remaining search/collection consumers; other provider raw views, cleanup and Windows qualification gates stay open.
