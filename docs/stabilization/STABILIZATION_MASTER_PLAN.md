...
- [ ] До первого vendor switch в 9B закрыть R36: полный manifest/version/hash/license/transitive-runtime inventory уже vendored и новых assets, zero-network checks, allowlisted local-worker lifecycle и explicit WASM/font policy через adapter boundary. [Evaluation](R32_VENDOR_UI_EVALUATION.md) не является разрешением подключить весь shortlist.
- [ ] `ViewerRegistry` остаётся UI-only dispatch над allowlisted kind/MIME и уже разрешённым bounded payload. Он не читает bridge/CAS/network и не вводит model-facing `{kind,title,content}` transport: модель сохраняет Tool Result v1 + revision-pinned `ResourceRef`, а viewer выбирает typed UI projection.
