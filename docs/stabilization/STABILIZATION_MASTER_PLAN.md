...
- [ ] До первого vendor switch в 9B закрыть R36: полный manifest/version/hash/license/transitive-runtime inventory уже vendored и новых assets, zero-network/worker/WASM/font checks и adapter boundary. [Evaluation](R32_VENDOR_UI_EVALUATION.md) не является разрешением подключить весь shortlist.
