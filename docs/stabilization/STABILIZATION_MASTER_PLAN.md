...
3. Не добавлять новые продуктовые функции во время стабилизации.
   Явное исключение пользователя 2026-09-08: ограниченное чтение PDF/текста/изображений
   из вложений Outlook через существующие resources/CAS/media. Это текущий slice
   Phase 11, не переход к Phase 12; Windows gates остаются открытыми.
   [Контракт](../resource-fabric.md#outlook-attachment-content-user-authorized-2026-09-08).
...
- [x] По отдельному запросу пользователя 2026-09-08: compact run text diff для retained VBA и authored text/HTML members. Typed source-pair projection и один заменяющий UI formatter; без Diff2Html/unified-diff evidence, нового store или live Office reads. [Точный scope и bounds](../conversation-protocol.md#run-text-changes). Windows/WebView2/controller qualification остаётся открытой; Phase 12 не начинается.
...
5. **11T — typed Office tools и удаление legacy host dispatch — admitted:**
   ...
   - [x] 11T2 — existing `excel.add_sheet`/`excel.rename_sheet` переведены на
   ...
   - [x] 11T4 — existing `excel.add_table` переведён на typed service/direct bound
   ...
   - [x] 11T8 — Outlook получил bound local `OutlookDocumentSession` для exact
     Inspector/mail или Explorer/folder; все пять existing reads/mutations
     переведены на exact native registrations, typed service и direct backend,
     public host branches/helpers и execution-time `ActiveInspector`/
     `ActiveExplorer` fallback удалены. Windows WQ-SESSION/WQ-OUTLOOK открыт.
...
16. **11O / R61 — mandatory all-tool contract correction:** audit every effective
    ...
    Each tool family switches atomically through the existing ToolRuntime together
    with every built-in skill body that teaches its ids/arguments, and deletes its
    replaced path; no alias, dual schema or hidden latest fallback. The source
    baseline includes all 35 conditional Common tools and nine built-in Common
    skills; host/custom property inventory remains part of execution. This
    user-requested stabilization contour is mandatory before final
    ...
    Host-neutral 11O1–11O7 completed Resources/Capabilities, planning, HTML,
    Prompt/Tool/Skill authoring, VBA/macro, the final minimal mode/host core and
    UI-only documentation/typed Library Test/layout. Final Windows/live-provider
    WQ remains before Phase 12.
