...
- The backup list is a projection of `backup.created` and the `before` side of `mutation.prepared`. There are no mutable backup JSON files or inline source copies.
...
On the next safe VBA access for the active document, runtime finds preparations without a terminal record and compares live module state with the recorded before/intended hashes. It appends `committed`, `not_applied`, or `unknown`. Recovery never retries a write, deletes a module, or restores a backup automatically.
