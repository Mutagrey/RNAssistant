...

{
  ...
  ["app-utils.js", "app-run-view-state.js", "app-agent-model.js", "app-agent-approval.js"].forEach(asset => {
    assert.ok(index.includes(asset + "?v=" + (["app-utils.js", "app-agent-approval.js"].includes(asset) ? "run-view-state-20260830-1" : asset === "app-agent.css" ? "run-overlap-20260907-1" : "chat-projection-20260907-1")), asset + " uses the atomic cutover cache key");
  });
  ...
  assert.ok(index.includes("app-messages.js?v=run-overlap-20260907-1"), "messages uses the transcript incremental cache key");
