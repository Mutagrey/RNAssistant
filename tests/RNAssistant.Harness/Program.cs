    ...
    internal static partial class Program
    {
        ...
        public static int Main(string[] args)
        {
            var tests = new List<HarnessTest>
            {
                ...
                Test("qualification: strict manifest and catalog", QualificationManifestAndCatalogAreStrict),
                Test("qualification: finite pause resume and verifier pass", QualificationRunnerPausesAndPassesFromVerifier),
                Test("qualification: missing verifier evidence blocks pass", QualificationRunnerRejectsNarrativePass),
                Test("qualification: failure runs cleanup without retry", QualificationRunnerRunsCleanupWithoutRetry),
                Test("qualification: mandatory event barriers", QualificationRunnerHonorsMandatoryEventBarriers),
                ...
                Test("tool run result: terminal states", ToolRunResultProjectsTerminalStates),
                Test("tool run result: runtime controls", ToolRunResultProjectsRuntimePauses),
                Test("tool run result: error metadata", ToolRunResultPreservesErrorMetadata),
                Test("tool run result: projection preserves execution", ToolRunResultProjectionCannotChangeExecution),
                Test("tool run result: missing execution record", ToolRunResultRejectsMissingExecutionRecord),
                ...
                Test("kernel: read inspection after unknown effect", KernelAllowsInspectionAfterUnknownEffect),
                ...
                Test("kernel: failed call after corrective success proceeds", KernelAllowsFailedCallAfterInterveningSuccess),
                Test("kernel: another failure does not reset failed calls", KernelDoesNotResetFailedCallsAfterAnotherFailure),
                Test("kernel: successful read does not reset failed calls", KernelDoesNotResetFailedCallsAfterSuccessfulRead),
                Test("kernel: unknown call remains non-repeatable", KernelNeverRepeatsUnknownCallAfterInterveningSuccess),
                Test("kernel: independent reads execute in order", KernelReadsAreSequentialAndBounded),
                Test("kernel: managed mutations batch sequentially", KernelAllowsManagedMutationBatches),
                Test("kernel: unknown batch mutation continues tail", KernelUnknownBatchMutationContinuesTail),
                ...
                Test("kernel: confirmation allowed after unknown effect", KernelAllowsConfirmationAfterUnknownEffect),
                ...
                Test("agent: resource media projection failure is explicit", AgentResourceMediaProjectionFailureIsExplicit),
                ...
                Test("conversation v5: status-free round trip", ConversationV4RoundTripsWithoutStatus),
                Test("conversation v5: reads all accepted history forms", ConversationHistoryReadsAcceptedForms),
                Test("conversation v5: rejects ambiguous history records", ConversationHistoryRejectsAmbiguousRecords),
                Test("conversation v5: rejects unknown root fields", ConversationV4RejectsUnknownRootFields),
                Test("conversation v5: rejects malformed JSON and extensions", ConversationV4RejectsMalformedJson),
                Test("conversation v5: requires exact call shape", ConversationV4RequiresExactCallShape),
                Test("conversation v5: requires callable authority", ConversationV4RequiresCallableAuthority),
                Test("conversation v5: wire has no call identity", ConversationV4KeepsIdenticalCallsWithoutIds),
                Test("conversation v5: batches reads and managed mutations", ConversationV4BatchesOnlyExplicitReadOnlyCalls),
                Test("conversation v5: validates arguments before acceptance", ConversationV4ValidatesArgumentsBeforeAcceptance),
                Test("conversation v5: bounds call count", ConversationV4BoundsCallCount),
                Test("conversation v5: schema agrees with parser and transport", ConversationV4SchemaMatchesParserAndWire),
                Test("conversation v5: schema exposes only callable tools", ConversationV4SchemaAllowsOnlyCallableTools),
                ...
                Test("model protocol: v5 rejects model ids and validates batch safety", ModelProtocolValidatesV4Context),
                ...
                Test("tool pack: R61 mode and host profiles are minimal", CallableToolPackUsesR61ModeHostProfiles),
                ...
                Test("completion guard: typed outcomes and metadata", RunSummaryUsesTypedOutcomesAndMetadata),
                Test("completion guard: typed continuation evidence", RunSummaryRequiresTypedContinuationEvidence),
                ...
                Test("agent: no-tool checkpoint continues", SimpleAgentNoToolCheckpointContinues),
                ...
                Test("agent: R61 routine Excel avoids schema discovery", SimpleAgentR61RoutineExcelAvoidsSchemaDiscovery),
                ...
                Test("shared context: publication and authority-filtered reads", SharedContextPublicationAndReads),
                Test("shared context: compaction publication and writer drift", CompactionPublishesSharedContext),
                ...
                Test("token estimate: request admission reserves", TokenEstimateRequestAdmissionReserves),
                ...
                Test("excel identity probe: helper contract is bounded", ExcelIdentityHelperProtocolIsBounded),
                Test("excel identity probe: WQ0 verifier requires full distinct matrix", ExcelWq0VerifierRequiresFullMatrix),
                ...
                Test("storage: verified CAS prefix streams full source", CasPrefixVerifiesFullSource),
                ...
                Test("document originals: survive origin deletion restart and GC", DocumentOriginalsSurviveChatDeletionAndRestart),
                Test("document originals: idempotent concurrent publication", DocumentOriginalPublicationIsIdempotentAndConcurrent),
                ...
                Test("excel find replace: native ownership and direct backend", ExcelFindReplaceUsesExactNativeOwnership),
                ...
                Test("excel sheet: bound document scope", ExcelSheetUsesBoundDocumentScope),
                Test("excel range mutation: native ownership and direct backend", ExcelRangeMutationUsesExactNativeOwnership),
                Test("excel range mutation: family semantics", ExcelRangeMutationPreservesFamilySemantics),
                Test("excel range mutation: dispatch fault classification", ExcelRangeMutationClassifiesDispatchFaults),
                Test("excel range mutation: bound document scope", ExcelRangeMutationUsesBoundDocumentScope),
                Test("excel table: native ownership and direct backend", ExcelTableUsesExactNativeOwnership),
                Test("excel table: creation semantics", ExcelTablePreservesCreationSemantics),
                Test("excel table: dispatch fault classification", ExcelTableClassifiesDispatchFaults),
                Test("excel table: bound document scope", ExcelTableUsesBoundDocumentScope),
                Test("excel chart: native ownership and direct backend", ExcelChartUsesExactNativeOwnership),
                Test("excel chart: family semantics", ExcelChartPreservesFamilySemantics),
                ...
                Test("outlook attachments: exact source pages drift and retention", OutlookAttachmentsRetainExactContent),
                Test("outlook attachments: capture bounds cancellation and mismatch", OutlookAttachmentCaptureGuards),
                ...
                Test("tool presentation: exact call changes", ToolPresentationCallChanges),
                Test("tool presentation: artifact call correlation", ToolPresentationArtifactCorrelation),
                Test("tool presentation: typed content blocks", ToolPresentationContentBlocks),
                ...
                Test("run changes: unverified intended previews", RunChangesIntendedPreviews),
                ...
                Test("tools: typed library mutations are revision guarded", ToolLibraryMutationsAreRevisionGuarded),
                ...
                Test("skills: references are revisioned and runtime-continued", SkillReferencesAreRevisionedAndRuntimeContinued),
                ...
                Test("vba: restore service owns workflow", VbaRestoreServiceOwnsWorkflow),
                ...
                Test("vba: mutation backend throw before effect", VbaMutationBackendThrowBeforeEffect),
                Test("vba: mutation committed after backend throw", VbaMutationCommittedAfterBackendThrow),
                Test("vba: mutation unavailable read-back is unknown", VbaMutationUnavailableReadBackIsUnknown),
                Test("vba: mutation read-back divergence is unknown", VbaMutationReadBackDivergenceIsUnknown),
                ...
                Test("vba: patch uses live state and whole write returns recovery", VbaPatchUsesLiveStateAndWholeWriteReturnsRecovery),
                ...
                Test("vba: rename recovery classifies complete states", VbaRenameRecoveryClassifiesCompleteStates),
                Test("vba: rename cancellation boundaries", VbaRenameCancellationBoundaries),
                ...
                Test("vba: write rejects duplicate procedures", VbaWriteRejectsDuplicateProcedures),
                ...
                Test("vba: patch rejects duplicate procedures", VbaPatchRejectsDuplicateProcedures),
                ...
                Test("vba: restore preserves exact source lineage", VbaRestorePreservesExactSourceLineage),
                ...
                Test("vba: package recovery is typed", VbaPackageRecoveryIsTyped),
                ...
                Test("vba: package terminal loss blocks orphan run", VbaPackageTerminalLossBlocksOrphanRun),
                Test("vba: package marker drift blocks run", VbaPackageMarkerDriftBlocksRun),
                Test("vba: package journal blocks stripped orphan marker", VbaPackageJournalBlocksStrippedOrphanMarker),
                Test("vba: package session install rejects probe race", VbaPackageSessionInstallRejectsProbeRace),
                Test("vba: package rechecks before macro", VbaPackageRechecksBeforeMacro),
                Test("vba: package catalog rejects extra component", VbaPackageCatalogRejectsExtraComponent),
                Test("vba: package prepare failure blocks dispatch", VbaPackagePrepareFailureBlocksDispatch),
                Test("vba: package backend throw is assessed", VbaPackageBackendThrowIsAssessed),
                Test("vba: package mutate-then-throw commits", VbaPackageMutateThenThrowCommits),
                Test("vba: package marker-only divergence is unknown", VbaPackageMarkerOnlyDivergenceIsUnknown),
                Test("vba: package CAS rejects post-prepare drift", VbaPackageCasRejectsPostPrepareDrift),
                Test("vba: package COM install guard rejects drift", VbaPackageComInstallGuardRejectsDrift),
                Test("vba: package read-back loss is unknown", VbaPackageReadBackLossIsUnknown),
                Test("vba: package cancellation before dispatch", VbaPackageCancellationBeforeDispatch),
                Test("vba: package cancellation after install cleans", VbaPackageCancellationAfterInstallCleans),
                Test("vba: package cleanup failure leaves blocked orphan", VbaPackageCleanupFailureLeavesBlockedOrphan),
                Test("vba: package source marker is reserved", VbaPackageSourceMarkerIsReserved),
                ...
                Test("resource cutover: retained payload failures are typed", ResourceRetainedPayloadsFailClosed),
                ...
                Test("resource cutover: fork preparation fails closed", ResourceForkPreparationFailsClosed),
                ...
                Test("tool editor: exact builtin documentation publication", ToolEditorReadsPublishedDocumentation),
                Test("tool editor: builtin documentation fails closed", ToolEditorDocumentationFailsClosed),
                ...
                Test("shared Markdown: lifecycle and competing chats", SharedMarkdownLifecycle),
                Test("shared Markdown: agent execution and replay", SharedMarkdownKernelReplay),
                Test("shared Markdown: prepared guards and batch identity", SharedMarkdownPreparedGuards),
                ...
                Test("resource cutover: raw stored admission", ResourceRawStoredAdmission),
                Test("resource cutover: raw workspace files", ResourceRawWorkspaceFiles),
