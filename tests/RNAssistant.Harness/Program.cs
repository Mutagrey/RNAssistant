    ...
    internal static partial class Program
    {
        ...
        public static int Main(string[] args)
        {
            var tests = new List<HarnessTest>
            {
                ...
                Test("kernel: duplicate call id within response", () => KernelRejectsDuplicateIds(false)),
                Test("kernel: duplicate call id across steps", () => KernelRejectsDuplicateIds(true)),
                ...
                Test("kernel: confirmation retains accepted ids", KernelConfirmationCannotReuseAcceptedId),
                ...
                Test("conversation v3: status-free round trip", ConversationV3RoundTripsWithoutStatus),
                Test("conversation v3: reads all accepted history forms", ConversationHistoryReadsAcceptedForms),
                Test("conversation v3: rejects ambiguous history records", ConversationHistoryRejectsAmbiguousRecords),
                Test("conversation v3: rejects unknown root fields", ConversationV3RejectsUnknownRootFields),
                Test("conversation v3: rejects malformed JSON and extensions", ConversationV3RejectsMalformedJson),
                Test("conversation v3: requires exact call shape", ConversationV3RequiresExactCallShape),
                Test("conversation v3: requires callable authority", ConversationV3RequiresCallableAuthority),
                Test("conversation v3: ids are unique across accepted run", ConversationV3RejectsAcceptedRunIdReuse),
                Test("conversation v3: batches only explicit read-only calls", ConversationV3BatchesOnlyExplicitReadOnlyCalls),
                Test("conversation v3: validates arguments before acceptance", ConversationV3ValidatesArgumentsBeforeAcceptance),
                Test("conversation v3: bounds call count", ConversationV3BoundsCallCount),
                Test("conversation v3: schema agrees with parser and transport", ConversationV3SchemaMatchesParserAndWire),
                Test("conversation v3: schema exposes only callable tools", ConversationV3SchemaAllowsOnlyCallableTools),
                ...
                Test("model protocol: v3 validates run ids and singleton safety", ModelProtocolValidatesV3Context),
