...
| Raw original attachment bytes / exact CAS / inert binary HTML delivery | `Program.ResourceAuthorityTests.cs`; shared browser consumers in `tests/web/resource-data-plane.test.js`, `tests/web/html-workspace-export.test.js` | `resource cutover: raw`, `html tools: native ownership and typed binding` |
