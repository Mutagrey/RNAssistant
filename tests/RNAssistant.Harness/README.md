...
| VBA editor exact Gateway snapshot, source transport and save guard | `Program.VbaEditorResourceTests.cs`, `Program.ResourceGatewayTests.cs`, `Program.ResourceAuthorityTests.cs`, `Program.ContextBridgeTests.cs`; UI in `tests/web/vba-resource-read.test.js` | `vba editor:`, `vba: editor module waits`, `resources: live Office and VBA`, `resource cutover: live and retained logical continuations`, `bridge: typed vba` |
