...
| Raw original attachment and stored artifact bytes / exact historical CAS / inert binary HTML delivery / no inline or broken-attachment fallback | `Program.ResourceAuthorityTests.cs`; shared browser consumers in `tests/web/resource-data-plane.test.js`, `tests/web/html-workspace-export.test.js` | `resource cutover: raw`, `html tools: native ownership and typed binding` |
