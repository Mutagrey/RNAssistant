...
| Binary view discovery, early admission and retained limits | `Program.ResourceAuthorityTests.cs`; existing image/PDF cases in `Program.ResourceGatewayTests.cs` | `resource cutover: canonical binary`, `resource cutover: binary capabilities`, `artifact viewer: exact image`, `artifact viewer: exact PDF` |
