...
| Physical/layer dependency direction | `Program.ProjectStructureTests.cs`: Core.Agent, ModelProtocol, VBA, resources, OfficeHosts and UI boundaries, explicit VBA marker contract, root application façade, plus production source inclusion | `architecture:`, `harness: production projects include all source files` |
