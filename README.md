...
In Agent mode the prompt contains bootstrap and currently loaded native-like tool schemas plus one compact `RUNTIME_CONTEXT.capabilities.items` catalog of exact tool and skill ids with an explicit `kind`. The full runnable catalog remains local. If the compact catalog is truncated, `common.capabilities_search` returns bounded metadata; `common.capabilities_read` loads either an exact tool schema or a complete skill body. A skill catalog entry is metadata only and counts as loaded only while active context contains matching complete evidence with top-level `data.loaded=true`, `data.complete=true`, and `data.truncated=false`. In strict response-schema mode, optional tool arguments are nullable; runtime treats synthetic `null` as omitted and applies code-owned schema defaults instead of forcing the model to invent values, while preserving `null` explicitly allowed by the original tool schema. The model returns one raw JSON object. A tool turn contains one or more calls:
...
Every response declares conversation-response v2 `status`. `in_progress` requires one or more calls. Terminal `completed`, `awaiting_user`, `blocked`, or `refused` requires an empty array, for example `{"status":"completed","message":"...","tool_calls":[]}`. Runtime never infers state from wording or punctuation. Agent mode uses the configured `json_object` (default) or strict runtime-generated `json_schema`; an explicitly rejected schema may fall back once, request-locally, to `json_object` when enabled. There are no native tool-call transport, planner state machine, router, skill activation, automatic tool retries, or separate verification phase. ModelProtocol permits `MaxAgentFormatRetries` total protocol responses per logical model step (default 10, range 1–20, including the first). Timeout/network/server failures have a separate budget of two retries per step, with cancellable 1s/2s delays; schema fallback is also separate and works during repair. The maximum is N+3 raw requests, at most 23. Every retry starts from the original accepted prompt and neither rejected output nor correction instructions enter chat history. See [retry policy](docs/conversation-protocol.md#retry-policy-phase-2b).
