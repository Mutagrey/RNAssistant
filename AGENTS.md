...
- Один model turn может выбрать не более одного внешнего tool. Fences, surrounding prose, конфликтующие envelopes, `function_call` и parallel tool calls не поддерживаются. Одиночный legacy reply/toolCalls допустим только через безопасную нормализацию.
