...
- Каждый Agent prompt содержит один `RUNTIME_CONTEXT`: document identity, все доступные tools в native-like function JSON, компактный каталог enabled skills (`id`, `name`, `description`, автоматический `revision` Markdown), chat context и artifact references. Полный Markdown выбранного skill загружается через `common.skills_read` и остаётся обычным tool result в истории. Skill считается загруженным только пока в активном model context есть успешный non-truncated read с теми же `id` и `revision`; после compaction или смены revision его нужно прочитать снова. Runtime не маршрутизирует запрос, не активирует skills и не ведёт скрытый phase/planner state.
