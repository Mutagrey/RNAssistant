...
- Ответ модели — один conversation-response v3 JSON object только `message + tool_calls`, без model-owned status и других root fields. IDs уникальны во всём accepted user run, включая confirmation; write/external/confirmation-required/unclassified calls — singleton, batch допускает только independent local reads. Пустой `tool_calls` завершает model loop, но не доказывает успешный effect. Native provider refusal хранится отдельно. Невалидные attempts не входят в replay/history; automatic tool retry и отдельной verification phase нет. Несовместимая история требует явного нового чата/reset до model preparation или подтверждения tool.
