...
- VBA live state принадлежит Office document. Единственный durable recovery source RNAssistant — document-scoped append-only `vba-journals/*/mutations.events.jsonl`; before/intended source и rollback backups хранятся в общем CAS. `write/patch/delete/restore` пишут prepared до COM и terminal после read-back, а незавершённые записи на следующем безопасном VBA-доступе только сверяются с live state и никогда автоматически не replay/restore.
