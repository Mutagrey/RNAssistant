...
- Tool result передаётся следующим user protocol message как `TOOL_RESULT:` и JSON `{ok, tool_call_id, name, status, message, data, error}`. После confirmation и success, и failure возвращаются модели; явный пользовательский cancel терминален для текущего run. Модель сама выбирает следующий шаг; runtime не делает automatic retry или отдельную verification phase.
...
- Безопасные public VBA tools (`vba_replace_text`, `vba_apply_patch`, create/delete/restore) доступны Agent mode, но требуют confirmation, backup и где применимо актуальный `expectedCodeSha256`. Низкоуровневые whole-module/macro backend tools остаются скрыты.
