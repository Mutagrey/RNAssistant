        ...
        public IEnumerable<SkillDefinition> GetBuiltInSkills()
        {
            return new[]
            {
                new SkillDefinition
                {
                    ...
                    BodyMarkdown = "# PowerPoint Deck Building\n\nUse this skill for creating, restructuring, or cleaning up a presentation. A multi-slide discovery -> construction -> verification workflow normally also uses `common.task_tracking`. Exact loaded tool schemas remain authoritative for arguments.\n\n## Workflow\n\n- Inspect an existing deck with `common.resources_find/read`; use an exact returned document target or `PowerPoint slide: N` (one-based). View `text` reads slide text; `source` includes text and speaker notes. Oversized captures require a smaller target; use `powerpoint.list_objects` before changing unknown shapes. Preserve the deck's theme, terminology, evidence, and requested slide order unless redesign is explicit.\n- Build one clear idea per slide. Use short titles, concise copy, readable hierarchy, consistent alignment, and a logical narrative such as context, evidence, recommendation, and next steps. Do not overload slides with paragraphs or fabricate facts.\n- Use `powerpoint.add_slide`, `powerpoint.set_text`, and `powerpoint.add_object` for deliberate creation; use `powerpoint.duplicate_slide` or `powerpoint.move_slide` only when the requested structure needs it. Use `powerpoint.search_text` / `powerpoint.replace_text` for bounded replacement and load `common.text_search_replace` for its safeguards.\n- Add speaker notes only when useful and supported by the exact current tool contract; never claim an unsupported edit.\n\n## Definition of done\n\nFinish only when requested slide content, object changes, and ordering have successful tool evidence. Re-read affected slides or objects when the mutation result does not itself prove the final state. State that visual composition still needs PowerPoint validation when no rendered evidence is available.",
                    Enabled = true,
                    BuiltIn = true
                }
            };
