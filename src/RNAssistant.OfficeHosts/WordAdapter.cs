        ...
        public IEnumerable<SkillDefinition> GetBuiltInSkills()
        {
            return new[]
            {
                new SkillDefinition
                {
                    ...
                    BodyMarkdown = "# Word Document Editing\n\nUse this skill for Word drafting, revision, review, structure, and formatting. A multi-stage rewrite normally also uses `common.task_tracking`. Exact loaded tool schemas remain authoritative for arguments.\n\n## Workflow\n\n- Read the relevant source with `common.resources_read`: discover the document or selection target through `common.resources_find`, or use an explicit `Word range: 0:1000` target (zero-based start, exclusive end); use `word.inspect` for headings, tables, comments, or statistics. Before a targeted edit, establish the exact selection or character range and preserve surrounding structure and the user's tone unless asked to change it.\n- Use `word.find_text` for literal/regex search over an exact main/selection/all story snapshot and `word.replace_text` for replacement. Search coordinates belong to the returned story and captured state, not permanently current text; re-search after changes. Search scope resources can also be read through `common.resources_read`; oversized scopes fail explicitly. Load `common.text_search_replace` for its safeguards. Use `word.write_text` only for a deliberate insertion, paragraph, or selection replacement. Use `word.format_text`, `word.add_table`, `word.insert_page_break`, and `word.add_comment` only for the matching requested outcome.\n- Keep content and formatting changes explicit. Do not replace the whole document when a bounded operation preserves more of the original. For review-only requests, report findings separately and do not mutate the document.\n\n## Definition of done\n\nFinish only when each requested content and formatting change has matching successful read-back evidence. Preserve names, dates, links, commitments, and untouched sections; state any layout or pagination detail that still needs visual Word validation.",
                    Enabled = true,
                    BuiltIn = true
                }
            };
