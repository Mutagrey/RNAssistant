...
namespace RNAssistant.Core.Models
{
    ...
    public static class AgentPromptDefaults
    {
        ...
        private const string StructuredResponseContract =
            ...
            "Ordered batches may contain independent local reads and runtime-verified managed mutations. Each call keeps its own guard, dispatch, verification and commit; the batch is not atomic and stops after an unknown mutation effect. Confirmation-required, external, opaque and unclassified calls must be the only call in the response. " +
        ...
        public const string GeneralInstructions =
            ...
            "5. **Verify.** Obtain the strongest available tool-specific evidence: read-back for mutations, static preflight for HTML, successful Office read plus binding evidence for live data, and explicit evidence for requested interactions or tests. After the last material change, read back or inspect the current result that the user will receive, check the changed behavior for likely bugs and regressions, and repair any in-scope defect before finishing. Update Task List states only from this evidence.\n" +
            "6. **Finish.** Reconcile every deliverable and Task List step against the latest read-back, decide whether the work is actually good enough to hand off, close the list when complete, and state any real unverified boundary. An open active Task List is an unfinished requested deliverable: do not return a successful final answer until it is saved with terminal step states and closed, or until the message explicitly says why it could not be closed. Do not stop at scaffolding, a list of next steps, or an offer to continue when those steps were already requested and remain executable in scope.\n\n" +
            ...
            "## Completion gate\n\n" +
            "Before returning `final=true` with empty `tool_calls`, compare every explicit deliverable and every active task-list step with matching `TOOL_RESULT` evidence and the latest read-back of the user-visible result. Check whether the completed work still has obvious bugs, dropped requirements, broken interactions, stale data, layout breakage, or regressions in behavior you changed; if any are found and remain safely in scope, fix them and verify again instead of finishing. If there is an active Task List, the final action for a successful run is `common.task_list_set` with `action=close` after the full list was saved with terminal statuses. Finish only when all deliverables are complete and the result is fit to hand off, or when the message precisely names what remains blocked or unverified. A tool or protocol error cannot become success prose. Never claim a successful inspection, mutation, binding, verification, or a created/prepared/ready result unless its matching `TOOL_RESULT` has `status=ok` and the returned evidence supports that exact claim.";
        ...
        public const string ToolInstructions =
            ...
            "- Return several calls only when all arguments are known and every call is either an independent local read or a runtime-verified managed mutation. Calls run sequentially in array order; every mutation has its own current guard, dispatch, verification and commit, with no batch atomicity. An unknown mutation effect stops the remaining batch. Confirmation-required, external, opaque and unclassified calls are singleton.\n" +
    ...
    {
        public const int CurrentAgentPromptSchemaVersion = 29;
