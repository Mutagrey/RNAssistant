        ...
        public static IReadOnlyList<SkillDefinition> GetSkills(IOfficeApplicationAdapter adapter)
        {
            var result = new List<SkillDefinition>
            {
                ...
                Skill(
                    ...
                    "Build and edit local HTML reports, dashboards, visual plans, CSS, scripts, static JSON, and refreshable Office-bound data. Prefer it when the chat HTML preference is enabled or visual presentation materially helps.",
                    "# HTML Workspace Authoring\n\n- Call common.html_workspace_read without arguments to list files and data sources. To read a body, pass resourceType=file|data and its exact name.\n- Use common.html_workspace_upsert for files and intentionally static editable JSON. Runtime infers HTML/CSS/JavaScript kind from a file extension. Updating a bound data source this way makes it static. Use common.html_workspace_delete for removal.\n- For Office-backed data that should stay current, call common.html_data_bind once with an approved read-only source tool and its native object arguments. Prefer transform=table for row arrays used by grids or charts; the result has columns, rows, and rowCount. Use raw when the source shape already suits the page.\n- HTML reads data from window.RNAssistantData[name]. Optional metadata and accessors are available through window.RNAssistantDataMeta and window.RNAssistant.data. Do not paste Office values into JavaScript when a binding is appropriate.\n- Use refreshPolicy=on_preview for dashboards expected to follow the document, or manual when repeat reads would be surprising or expensive. Call common.html_data_refresh after an Office change when the current answer depends on fresh values. Call common.html_data_freeze to keep current JSON and remove the binding.\n- Keep pages local and editable. Split substantial CSS and JavaScript into separate files; independent reasonably sized resources may be written in the same tool turn. Default to a responsive full-page layout with body margin 0; do not force a narrow centered card unless requested.\n- Network fetch is allowed only through the RNAssistant host after the user explicitly allows the HTTP(S) origin. Never use mode:no-cors or embed API keys and credentials.")
            };
