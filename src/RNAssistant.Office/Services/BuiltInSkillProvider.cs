        ...
        public static IReadOnlyList<SkillDefinition> GetSkills(IOfficeApplicationAdapter adapter)
        {
            var result = new List<SkillDefinition>
            {
                ...
                Skill(
                    ...
                    "# VBA Code Editing\n\n- Use the common.vba_* tools in Excel, Word, and PowerPoint. Start with common.vba_list_modules, then use common.vba_read_module for small modules or common.vba_read_lines for the exact range needed. Use common.vba_search_code for project-wide literal or regex search.\n- Read or search the target before every edit/delete. Runtime binds the exact observed code snapshot and checks it again after confirmation; expectedCodeSha256 is not a tool argument. If the snapshot is stale, read again and review the changed code.\n- Prefer common.vba_apply_patch and pass patch as a native JSON array, never as a string containing JSON. Combine all already-known edits to one module into one ordered patch array; never return several patch calls for the same snapshot. Use text for replacement content; replace is also accepted as its alias. replace requires exactly one match; replaceAll is explicit; insertBefore/insertAfter insert whole line-safe blocks around a non-empty unique anchor and must never be used to replace existing code. replaceLines is one-based and each operation sees the result of preceding operations. A single trailing newline in replacement text is treated as a terminator, not an extra blank line. regexReplace supports capture groups and a replacement limit.\n- After common.vba_read_lines, prefer replaceLines over copying a large multi-line fragment into common.vba_replace_text. Set replaceAll=true only after checking every occurrence. Never embed raw control characters in source; generate them at VBA runtime with ChrW$(n).\n- A failed safe mutation is not permission to bypass it with hidden whole-module or macro execution. Inspect the error, re-read current code, retry with a safe public tool, or explain the blocker.\n- Every mutation requires confirmation. Edits and deletes create a rollback backup; create verifies the final component and removes an incomplete component on failure. Results return the actual read-back code hash; VBE-only case, spacing, and terminal-line normalization is accepted.\n- Create supports StdModule, ClassModule, and a blank MSForm/UserForm with code-behind. Visual UserForm controls, layout, properties, and FRX assets are outside these tools. Delete supports only StdModule and ClassModule; document modules and UserForms cannot be deleted. Restore only an id returned by common.vba_list_backups."),
                Skill(
