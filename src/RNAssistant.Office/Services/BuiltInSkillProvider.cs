        ...
        public static IReadOnlyList<SkillDefinition> GetSkills(IOfficeApplicationAdapter adapter)
        {
            var result = new List<SkillDefinition>
            {
                ...
                Skill(
                    ...
                    "- Discover VBA through the shared resource tools: list provider `vba` with kind omitted for the project plus live components, or use kind `vba-backup` only for backups. Use `common.resources_search` for bounded literal discovery, then read an exact returned component URI with representation `source`. Continue only with that read's `nextCursor`. On `resource_revision_changed`, restart the same URI/representation with both cursor and revision omitted; on `resource_cursor_invalid`, omit cursor. Never guess a resource URI or use removed VBA read/search aliases.\n" +
