        ...
        public static IReadOnlyList<SkillDefinition> GetSkills(IOfficeApplicationAdapter adapter)
        {
            var result = new List<SkillDefinition>
            {
                Skill(
                    ...
                    "# Task Tracking\n\nCreate a task list for work with at least three meaningful user-level stages. Do not turn individual reads or tool calls into artificial tasks. Loading this skill does not load the task-list tool schemas: before first use, load each needed exact tool id through common.capabilities_read and wait for its successful schema result.\n\n- Call common.task_list_create before execution with stable ordered step ids and at most one in_progress step.\n- The list tracks execution; it is not a strategic design document, execution authority, or proof of success.\n- After material progress, call common.task_list_update with the complete step list. Preserve ids and mark completed only after matching TOOL_RESULT status=ok and evidence that the requested work is complete. Tool success does not by itself prove an applied effect; a verified no-op may also be ok.\n- If the active list is outside context, find it with common.resources_find in the conversation scope and read the exact returned semantic target. Never pass a resource URI, revision, or cursor.\n- Keep blocked work visible. Before a successful final answer, make all steps terminal and call common.task_list_close with completed. Use cancelled or superseded when appropriate.\n- Do not update after every trivial tool call and do not expose hidden reasoning in task text."),
                Skill(
                ...
                Skill(
                    ...
                    "- Discover live code with `common.resources_find` and scope `vba`; use scope `backups` only for recovery evidence. Read the exact returned semantic target with representation `source`. After `hasMore=true`, continue that same target with action `next`; after `resource_revision_changed`, restart it with action `read`. URI, revision, cursor, provider, and provider-specific kind are runtime-owned.\n" +
                ...
                Skill(
                    ...
                    "- Tools are executable actions, not guidance documents. Only manifest-based VBA custom tools are supported.\n" +
                    ...
                    "- Advanced parameters is an actual JSON object, and components is an actual array of VBA sources; never double-encode them as JSON strings.\n" +
                ...
                Skill(
                    ...
                    "# VBA Tool Authoring\n\nUse this skill only for reusable VBA tools. Prefer an existing built-in tool first.\n\n" +
