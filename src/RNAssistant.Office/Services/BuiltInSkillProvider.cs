        ...
        public static IReadOnlyList<SkillDefinition> GetSkills(IOfficeApplicationAdapter adapter)
        {
            var result = new List<SkillDefinition>
            {
                ...
                Skill(
                    ...
                    "# HTML Workspace Authoring\n\n- Call common.html_workspace_read without arguments to list files/data sources. To read a body, pass resourceType=file|data and its exact name.\n- Use common.html_workspace_upsert for both files and editable JSON data; runtime infers HTML/CSS/JavaScript kind from a file extension and creates missing resources. Use common.html_workspace_delete with the same resourceType/name pair for removal.\n- Keep pages local and editable. Split substantial CSS and JavaScript into separate files; independent reasonably sized resources may be written in the same tool turn.\n- Default to a responsive full-page layout with body margin 0; do not force a narrow centered card unless requested.\n- Network fetch is allowed only through the RNAssistant host after the user explicitly allows the HTTP(S) origin. Never use mode:no-cors or embed API keys and credentials.")
            };
