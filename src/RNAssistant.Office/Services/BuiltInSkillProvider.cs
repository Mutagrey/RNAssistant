        ...
        public static IReadOnlyList<SkillDefinition> GetSkills(IOfficeApplicationAdapter adapter)
        {
            var result = new List<SkillDefinition>
            {
                ...
                Skill(
                    ...
                    "VBA code editing",
                    "Safely inspect, create, refactor, patch, delete, or restore VBA macros and components in Excel, Word, or PowerPoint. Use whenever a request changes VBA source.",
                    "# VBA Code Editing\n\n" +
                    "## Choose the operation\n\n" +
                    "- Use only the compact `common.vba_*` facade. Call `common.vba_read_module` without `moduleName` to list components, with a name for bounded whole source, or with `startLine`/`lineCount` for an exact range. Use `common.vba_search_code` for cross-module discovery, not as a mandatory precondition.\n" +
                    "- Use `common.vba_write_module` only when you have the complete intended module source. Default `mode=upsert` updates or creates; `componentType` matters only on creation. Use `createOnly` or `updateOnly` only when existence itself must be guarded. Continue with the actual normalized name returned by the tool.\n" +
                    ...
                    "- Rename is not part of write or patch. A different missing `moduleName` creates another component; never imitate rename with write plus delete because that is not one guarded/journaled transaction. Explain that atomic rename is unavailable.\n\n" +
                    "## Patch without corrupting code\n\n" +
                    "- Pass `patch` as a native JSON array, never stringified JSON. Combine known edits for one module into one ordered array; every later operation sees the result of earlier operations. Use only the fields declared by the selected operation and use `text` for replacement content.\n" +
                    "- Literal anchors are exact and case-sensitive, including whitespace. Prefer `replace`, which requires one match. Use `replaceFirst` only when the first occurrence is intentionally sufficient, `replaceAll` only after considering every match, and `regexReplace` only when a bounded pattern is genuinely clearer.\n" +
                    "- `insertBefore` and `insertAfter` insert before or after the complete source line containing one unique anchor. A partial-line anchor does not split the VBA statement. Use insertion for adjacent declarations or procedures; use replacement to change the anchored line itself.\n" +
                    "- `replaceLines` uses one-based `startLine` and `deleteCount`. Prefer it when an immediately preceding bounded read gives exact coordinates; prefer a unique semantic insert anchor when line numbers are unavailable or surrounding lines may move. Appending at the logical line count plus one is valid; a final source newline is a terminator, not an extra editable line.\n" +
                ...
                Skill(
                    ...
                    "- Call common.tools_read with id only when existing fields must be inspected, or without id for a compact custom-tool list.\n" +
                ...
                Skill(
                    ...
                    "# Skill Authoring\n\nUse this skill when the user asks to create or edit RNAssistant guidance.\n\nA skill is a concise Markdown instruction file. It describes workflow, constraints, and preferred tools; it never executes actions itself. Agent context contains metadata only: id, name, description, package revision, body size, and reference count. A listed entry is not loaded guidance. Load the complete Markdown body through common.skills_read when the user names the skill or its catalog description matches the task, and reload it when matching data.loaded=true evidence is no longer fully present in active context. Keep the core body under 500 lines and split broad workflows into narrower skills. Put detailed UTF-8 Markdown directly under references/, link each file from SKILL.md, and explain when to read it. Read only needed chunks with common.skills_read referencePath/offset/maxChars. Use common.skills_upsert with referencePath and referenceMarkdown to create or replace one reference, and common.skills_delete with referencePath to delete one. Mutate the core and a reference in separate confirmed calls. If the requested skill needs a new executable capability, create and validate a tool separately instead of embedding executable logic in the skill. For the core, common.skills_upsert creates a missing id or preserves omitted fields on update. Read existing content only when the requested edit depends on it. Built-in skill ids are reserved. Never store secrets or weaken runtime safety.\n\nRecommended sections:\n\n- Purpose\n- Workflow\n- Constraints\n- Useful tools"),
                Skill(
                    "common.prompt_authoring",
                    "Prompt authoring",
                    "Review and improve RNAssistant editable prompts without weakening its protocol or safety.",
                    "# Prompt Authoring\n\nUse this skill only when the user asks to inspect or improve RNAssistant prompts.\n\n- Call common.prompts_read with includeDefaults=true before proposing a change.\n- Agent instructions are composed in stable order from SystemPrompt (general contract), AgentToolsPrompt, and AgentSkillsPrompt, followed by one dynamic RUNTIME_CONTEXT. Keep cross-tool policy in AgentToolsPrompt and tool-specific inputs/errors in each tool description. The skill catalog is metadata only; AgentSkillsPrompt must require common.skills_read and exact loaded evidence before skill-governed work.\n- Other editable Markdown prompts are ChatSystemPrompt, ContextCompactionPrompt, ChatTitlePrompt, and AttachmentAnalysisPrompt. Compatibility probes are intentionally fixed so their diagnostics remain trustworthy.\n- Preserve the minimal response fields message and tool_calls. An empty tool_calls array is terminal and cannot accompany unfinished progress; each actual call needs a unique id, exact name, and object arguments. Independent calls may be returned together and execute sequentially.\n- Chat mode has no tools. ContextCompactionPrompt owns durable summary criteria.\n- Do not weaken confirmation, secret handling, or the rule that tool success is established only by TOOL_RESULT ok=true.\n- common.prompts_save changes local settings and requires confirmation."),
                Skill(
                    ...
                    "# HTML Workspace Authoring\n\n## Inspect and edit\n\n" +
                    "- For an existing workspace, call common.html_workspace_read without arguments once for the compact manifest. Do not read every body. Use common.html_workspace_search for discovery and startLine/lineCount reads for only the needed source range.\n" +
