    ...
    internal static partial class VbaToolCatalog
    {
        ...
        internal static IEnumerable<ToolCatalogEntry> GetTools()
        {
            ...
            yield return Projection(WriteModule,
                "Mutates document: Write one complete VBA component source. Requires moduleName+code and uses mode=upsert/createOnly/updateOnly; componentType applies only on creation. Consolidate all changes derived from one fresh source snapshot into one complete write. Runtime rejects unsafe non-VBA syntax, export headers, hidden/control characters, joined block terminators, and duplicate procedure/property declarations before dispatch. Runtime binds current state before confirmation and verifies exact source/type read-back. After a prior source mutation of this module, read its complete current source, reconcile, and only then retry. Use common.vba_rename_module for an identity-preserving rename.",
