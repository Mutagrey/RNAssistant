...
async function addAttachmentFiles(files) {
  ...
  if (files.some(function (file) { return file.type.indexOf("image/") === 0; }) && activeModelSupportsImages() === false) {
    log("Выбранная модель не поддерживает изображения.");
    return;
  }
  if (files.some(function (file) { return file.type.indexOf("image/") === 0; }) && activeModelSupportsImages() === null) {
